...
# print ('./videos/{}.mp4'.format(FILE_NAME))
cap = cv2.VideoCapture('./videos/{}.mp4'.format(FILE_NAME)) # 0 for web camera input
# cap = cv2.VideoCapture('amazon.mp4')
...
writer = cv2.VideoWriter('./videos/{}_mediapipe.mp4'.format(FILE_NAME), fmt, frame_rate, size) # ライター作成
...
angles = {'TimeStamp': T, 'LShoulderRoll': LSR, 'LElbowRoll': LER,
          ...
          'LElbowYaw': LEY, 'RElbowYaw': REY}
# removed 'HipRoll': HPR, 'HipPitch': HPP
ThetaR = pd.DataFrame.from_dict(angles)
Theta = pd.DataFrame(columns=['TimeStamp', 'LShoulderRoll', 'LElbowRoll',
          'RShoulderRoll', 'RElbowRoll', 'HeadYaw',
          'HeadPitch', 'LShoulderPitch', 'RShoulderPitch',
          'LElbowYaw', 'RElbowYaw'])
          
# 0が入っているTimeStampを除く
for i in range(len(ThetaR.columns)):
    if np.any(ThetaR.loc[i].values == 0.0):
        pass
    else:
        Theta = Theta.append(ThetaR.loc[i])

cwd = os.getcwd()
ThetaR.to_csv(cwd + '\joint_data\{}_3d.csv'.format(FILE_NAME), index=False)
#
# plots = pd.read_csv('joint_angles.csv')
#
# # Plot the joint angles
# def JointAngles(plots):
#
#     fig, axs = plt.subplots(4, 3)
#     axs[0, 0].plot(plots['TimeStamp'], plots['RShoulderRoll'], 'tab:blue')
#     axs[0, 0].set_title('RShoulderRoll')
#     axs[1, 0].plot(plots['TimeStamp'], plots['LShoulderRoll'], 'tab:orange')
#     axs[1, 0].set_title('LShoulderRoll')
#     axs[2, 0].plot(plots['TimeStamp'], plots['RShoulderPitch'], 'tab:green')
#     axs[2, 0].set_title('RShoulderPitch')
#     axs[3, 0].plot(plots['TimeStamp'], plots['LShoulderPitch'], 'tab:red')
#     axs[3, 0].set_title('LShoulderPitch')
#
#     axs[0, 1].plot(plots['TimeStamp'], plots['RElbowRoll'], 'tab:blue')
#     axs[0, 1].set_title('RElbowRoll')
#     axs[1, 1].plot(plots['TimeStamp'], plots['LElbowRoll'], 'tab:orange')
#     axs[1, 1].set_title('LElbowRoll')
#     axs[2, 1].plot(plots['TimeStamp'], plots['RElbowYaw'], 'tab:green')
#     axs[2, 1].set_title('RElbowYaw')
#     axs[3, 1].plot(plots['TimeStamp'], plots['LElbowYaw'], 'tab:red')
#     axs[3, 1].set_title('LElbowYaw')
#
#     axs[0, 2].plot(plots['TimeStamp'], plots['HeadYaw'], 'tab:blue')
#     axs[0, 2].set_title('HeadYaw')
#     axs[1, 2].plot(plots['TimeStamp'], plots['HeadPitch'], 'tab:orange')
#     axs[1, 2].set_title('HeadPitch')
#     axs[2, 2].plot(plots['TimeStamp'], plots['HipRoll'], 'tab:green')
#     axs[2, 2].set_title('HipRoll')
#     axs[3, 2].plot(plots['TimeStamp'], plots['HipPitch'], 'tab:red')
#     axs[3, 2].set_title('HipPitch')
#
#     for ax in axs.flat:
#         ax.set(xlabel='Time', ylabel='Joint Angle')
#     for ax in axs.flat:
#         ax.label_outer()
#
# JointAngles(plots)
