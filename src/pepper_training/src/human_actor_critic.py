...
from utility import load_results_and_experiences, select_action, save_fig, compute_returns, compute_MSE_joint_reward
import settings
...
def trainIters(actor, critic, file_name_end):
  ...
  for nepisode in range(len(cumulative_rewards), settings.nvideos):
    ...
    # print(file_name_end)
    human_env_controller = HumanEnvController(file_name_end, nepisode)
    state = pepper_env_controller.get_state()
    human_state = human_env_controller.get_state(state_names)
    ...
    for i in range(human_env_controller.step_size):
      ...
      pepper_env_controller.publish_action_and_reward(action, -mse_human_pepper_joint)  # TODO NEP
      # pepper_env_controller.publish_action_and_reward(action)
